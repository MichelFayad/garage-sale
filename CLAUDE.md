...
- **Post-cutover follow-ups (both ✅ done after the migration):** (1) ✅ mobile CI — a `mobile` job (`subosito/flutter-action@v2`, Flutter 3.44.6) runs `flutter analyze --fatal-infos` + `flutter test`; the pre-existing 38 analyzer warnings/infos were cleaned first so the strict gate passes. (2) ✅ Expo-token hardening — `sendPush` now recognizes legacy `ExponentPushToken[...]`/`ExpoPushToken[...]` tokens, deletes them up front, and sends only real FCM tokens (they'd otherwise be rejected by `sendEachForMulticast` and never pruned). **Still open (larger, blocked — not quick fixes):** OAuth (Google/Apple/Facebook) was never ported to Flutter (needs provisioned console credentials); camera/library photo upload is backend-blocked (no blob storage); no emulator/device smoke test has been run (none available).
