...
1. **react/react-dom version mismatch.** `apps/web/package.json` had `"react-dom": "^19.0.0"` while the then-still-present RN app pinned `react@19.0.0` exactly. pnpm hoists a single React version across the workspace, so `react` got forced to `19.0.0` while `react-dom` floated to `19.2.7` under the same `^19.0.0` range — a version-mismatch crash on every page. Fixed by pinning `react-dom` to the exact `19.0.0` in `apps/web/package.json`. (The RN app has since been deleted in the Flutter cutover — the pin is retained for stability but the cross-app constraint that forced it is gone.) Check with `pnpm why react-dom` / `pnpm why react` — resolved versions must match exactly.
